...
const handleSubmit = async () => {
  if (formData.value.section_ids.length === 0) {
  ...
  const payload = {
    ...formData.value,
    section_ids: formData.value.section_ids.join(","), // convert array to string
  };
    ...
    .then((response) => {
      if (response.status == 201) {
        ...
        toast.success(
          ...
          `The subject ${formData.value.subject_name} has been created successfully.`
        );
