    ...
    <nav class="nav-items">
      <template v-for="item in navigationItems" :key="item.name">
        ...
        <router-link v-if="!item.children" :to="item.path" class="nav-item"
          :class="{ active: $route.name === item.name }" @click="closeSidebarOnMobile">
        ...
        <div v-else class="nav-item-wrapper">
          ...
          }" @click="toggleDropdown(item.name)" role="button" aria-expanded="false">
            <svg class="nav-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" :d="item.icon" />
            </svg>
            <span>{{ item.label }}</span>
            <svg class="dropdown-toggle-icon" :class="{ rotated: openDropdown === item.name }" width="16" height="16"
          ...
          <div class="collapse" :class="{ show: openDropdown === item.name }">
            <div class="nav flex-column">
              <router-link v-for="child in item.children" :key="child.name" :to="child.path"
                class="nav-link nav-link-sub" :class="{ active: $route.name === child.name }"
                @click="closeSidebarOnMobile">
                <svg class="nav-icon-sub" fill="none" stroke="currentColor" viewBox="0 0 24 24">
